...
- **Um `ClusterSecretStore` só, por cluster** (`k8s/external-secrets/cluster-secret-store.yaml`),
...
- `k8s/external-secrets/cluster-secret-store.yaml` + `external-secret.yaml`.
...
- IAM: usuário `eso-dev-secrets-reader` + policy `ESODevSecretsManagerRead` (criados via AWS
  CLI, sem material de credencial).
- Helm: `external-secrets/external-secrets` instalado no namespace `external-secrets`.
